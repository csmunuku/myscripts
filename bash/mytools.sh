...
# mytools.sh
#########################################################################################
# takes two files as arguments. file1 and file2
ts="`date '+%b_%d_%H%M%S'`"
if [ $# -ne 2 ]; then
   echo "Please specify two files as arguments. \"file1\" is your source and \"file2\" is your destination file"
   exit 1
fi
   file1="$1"
   file2="$2"
   ...
   if [ -f $file2 ]; then
      echo "\"$file2\" file already exists in your current directory"
      echo "Do you want to overwrite this file - (yes or no)?:"
      read input
      if [ ${input} = "yes" -o ${input} = "y" -o ${input} = "Y" -o ${input} = "YES" ]; then
         echo "Making a backup of \"$file2\" for you"
         cp -p $file2 ${file2}_$ts
      else
         echo "Please provide another filename as your destination file:"
         read newfile2
         file2="${newfile2}"
      fi
   fi
   echo "Create a File \"$file2\" from \"$file1\" By Changing"
   ...
   echo -e "48) Copy only portion of \"$file1\" to \"$file2\" by providing line numbers"
   echo -e "49) Change all Character in \"$file1\" to Lower case"
   echo -e "50) Change all Character in \"$file1\" to Upper case"
   echo -e "51) Remove all empty lines in \"$file1\" and write the output to \"$file2\""
